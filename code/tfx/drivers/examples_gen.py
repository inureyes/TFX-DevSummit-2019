...
class ExamplesGen(BaseDriver):
  ...
  def _import_artifact(self, single_input, uri):
    single_input.set_artifact_uri(uri)
    single_input.set_artifact_span(self._generate_span_or_fail(uri))
  ...
  def _prepare_input_for_processing(self, input_dict):
    ...
    for single_input in input_dict.values():
      ...
      #       not available.
      # If there is not registered artifact, use a new uri that has the smallest
      # span number.
      if registered_artifacts_in_dir:
        ...
        if candidate_new_uris:
          ...
          single_input.set_artifact(registered_artifact_in_dir_with_latest_span)
          self._logger.info('No valid new input, use previous input: {}'.format(
              registered_artifact_in_dir_with_latest_span))
      else:
        new_uri_to_process = min(
